...
void direct_serial_command()
{
    switch (CONSOLE_SERIALLink.read())
          ...
          case 'E': // receive command button commands
                  ...
                  Etmp = CONSOLE_SERIALLink.read();
                  while (CONSOLE_SERIALLink.available() == 0) {}
                  Eoffset = word(CONSOLE_SERIALLink.read(), Etmp);
          ...
          case 'W': // receive new VE obr constant at 'W'+<offset>+<newbyte>
                ...
                uint16_t Woffset;
                while (CONSOLE_SERIALLink.available() == 0) {}
                Wtmp = CONSOLE_SERIALLink.read();
                while (CONSOLE_SERIALLink.available() == 0) {}
                Woffset = word(CONSOLE_SERIALLink.read(), Wtmp);
                while (CONSOLE_SERIALLink.available() == 0) {}
                direct_receiveValue(Woffset, CONSOLE_SERIALLink.read());
          break;
          ...
          case 'r': 
                ...
                cmd = CONSOLE_SERIALLink.read();
                      ...
                      while (CONSOLE_SERIALLink.available() == 0) {}
                      offset = word(CONSOLE_SERIALLink.read(), tmp);
                      while (CONSOLE_SERIALLink.available() == 0) {}
                      tmp = CONSOLE_SERIALLink.read();
                      if (cmd != 87)          //if is "W" only 1 more byte is sent
                      {
                      while (CONSOLE_SERIALLink.available() == 0) {}
                      length = word(CONSOLE_SERIALLink.read(), tmp); 
                      }
                      else{length = tmp;}
...
void direct_sendValues(uint16_t offset, uint16_t packetLength, uint8_t cmd)
{
  ...
  fullStatus[23] = lowByte(currentStatus.GPSspeedo);//        
  fullStatus[24] = highByte(currentStatus.GPSspeedo);
  fullStatus[25] = lowByte(currentStatus.oil_Temp);//        
  fullStatus[26] = highByte(currentStatus.oil_Temp);
  fullStatus[27] = currentStatus.testIO_hardware;
  fullStatus[28] = lowByte(currentStatus.digIn);
  fullStatus[29] = highByte(currentStatus.digIn);
  fullStatus[30] = lowByte(currentStatus.digOut);    
  fullStatus[31] = highByte(currentStatus.digOut);
  fullStatus[32] = lowByte(currentStatus.Analog[1]);
  fullStatus[33] = highByte(currentStatus.Analog[1]);
  fullStatus[34] = lowByte(currentStatus.Analog[2]);
  fullStatus[35] = highByte(currentStatus.Analog[2]);
  fullStatus[36] = lowByte(currentStatus.Analog[3]);
  fullStatus[37] = highByte(currentStatus.Analog[3]);
  fullStatus[38] = lowByte(currentStatus.Analog[4]);
  fullStatus[39] = highByte(currentStatus.Analog[4]);  
  fullStatus[40] = lowByte(currentStatus.Analog[5]);
  fullStatus[41] = highByte(currentStatus.Analog[5]);
  fullStatus[42] = lowByte(currentStatus.Analog[6]);
  fullStatus[43] = highByte(currentStatus.Analog[6]);
  fullStatus[44] = lowByte(currentStatus.Analog[7]);
  fullStatus[45] = highByte(currentStatus.Analog[7]);
  fullStatus[46] = lowByte(currentStatus.Analog[8]);
  fullStatus[47] = highByte(currentStatus.Analog[8]);
  fullStatus[48] = lowByte(currentStatus.Analog[9]);
  fullStatus[49] = highByte(currentStatus.Analog[9]);
  fullStatus[50] = lowByte(currentStatus.Analog[10]);
  fullStatus[51] = highByte(currentStatus.Analog[10]);
  fullStatus[52] = lowByte(currentStatus.Analog[11]);
  fullStatus[53] = highByte(currentStatus.Analog[11]);
  fullStatus[54] = lowByte(currentStatus.Analog[12]);
  fullStatus[55] = highByte(currentStatus.Analog[12]);  
  fullStatus[56] = lowByte(currentStatus.Analog[13]);
  fullStatus[57] = highByte(currentStatus.Analog[13]);
  fullStatus[58] = lowByte(currentStatus.Analog[14]);
  fullStatus[59] = highByte(currentStatus.Analog[14]);
  fullStatus[60] = lowByte(currentStatus.Analog[15]);
  fullStatus[61] = highByte(currentStatus.Analog[15]);
  fullStatus[62] = lowByte(currentStatus.Analog[16]);
  fullStatus[63] = highByte(currentStatus.Analog[16]);    
